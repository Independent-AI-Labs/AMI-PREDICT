...
class ExperimentTracker:
    ...
    def log_epoch(self, epoch: int, train_loss: float, val_loss: float,
                  ...
                  epoch_time: float, lr: Optional[float] = None):
        """Log epoch results."""
    ...
    def _update_best_model(self):
        ...
        best = max(self.all_results, 
                  key=lambda x: x.get('summary', {}).get('best_val_acc', 0))
    ...
    def compare_experiments(self, metric: str = 'val_acc') -> pd.DataFrame:
        ...
        if not df.empty:
            df = df.sort_values(f'val_acc', ascending=False)
...
if __name__ == "__main__":
    ...
    exp_id = tracker.start_experiment(
        name="Test TCN",
        model_type="TCN",
        hyperparams={'hidden_size': 128, 'layers': 3},
        description="Testing experiment tracker"
    )
