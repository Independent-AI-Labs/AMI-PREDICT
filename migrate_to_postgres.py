...
PG_HOST = '172.72.72.2'
PG_PORT = 5432
PG_DB = 'postgres'  # Use default database
PG_USER = 'postgres'
PG_PASS = 'postgres'
...
def migrate_data():
    ...
    while offset < total_records:
        ...
        logger.info(f"Migrated {migrated:,}/{total_records:,} records "
                   f"({migrated/total_records*100:.1f}%) - "
                   f"Speed: {speed:.0f} rec/sec - ETA: {eta/60:.1f} min")
...
def test_parallel_query():
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        ...
        for future in as_completed(futures):
            ...
            logger.info(f"{symbol}: {count:,} records from {min_ts} to {max_ts} "
                       f"(query time: {elapsed:.2f}s)")
