...
def _bar_chart(width: int, height: int, pad: int, labels: list[str], values: list[float], title: str, value_fmt: str = "{v:.2f}%") -> str:
    ...
    for i, (lab, v) in enumerate(zip(labels, values, strict=False)):
        ...
        svg.append(
            f'<text x="{bx + bar_w/2:.1f}" y="{y - 6:.1f}" text-anchor="middle" font-family="sans-serif" font-size="11" fill="#333">{value_fmt.format(v=v)}</text>'
        )
        svg.append(
            f'<text x="{bx + bar_w/2:.1f}" y="{height - pad + 16}" text-anchor="middle" font-family="sans-serif" font-size="11" fill="#555">{lab}</text>'
        )
