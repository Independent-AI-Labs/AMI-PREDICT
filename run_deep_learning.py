...
try:
    ...
    if hasattr(torch, "xpu") and torch.xpu.is_available():
        ...
        print("PyTorch available. Using CUDA GPU")
    else:
        DEVICE = torch.device("cpu")
        print("PyTorch available. Using CPU")
except ImportError:
...
class DeepLearningExperiment:
    ...
    def load_data(self, symbol: str, timeframe: str) -> pd.DataFrame:
        ...
        try:
            ...
            data = []
            for row in session.query(MarketData).filter(MarketData.symbol == symbol, MarketData.timeframe == timeframe).order_by(MarketData.timestamp):
                data.append(
                    {
    ...
    def prepare_sequences(self, df: pd.DataFrame, seq_length: int = 60):
        ...
        feature_cols = ["returns", "log_returns", "volume_change", "open_norm", "high_norm", "low_norm", "volume_norm", "rsi"]
...
def main():
    ...
    print("Results saved to dl_results_*.json files")
