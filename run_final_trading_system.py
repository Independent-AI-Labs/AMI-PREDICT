    ...
    def backtest(self, df: pd.DataFrame, signals: pd.Series) -> dict[str, Any]:
        ...
        for i in range(len(signals)):
            ...
            if signal == 1 and position == 0:
                ...
                trades.append({"entry": entry_price, "exit": exit_price, "pnl": pnl, "return": (exit_price - entry_price) / entry_price})
...
def run_final_system():
    ...
    # Save results
    output = {"timestamp": datetime.now().isoformat(), "system": "FinalTradingSystem", "results": results}
