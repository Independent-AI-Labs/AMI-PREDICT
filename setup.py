...
def install_dependencies(python_exe):
    ...
    try:
        ...
        for gpu in c.Win32_VideoController():
            if gpu.Name and "Intel" in gpu.Name and "Arc" in gpu.Name:
                ...
                run_command(f"{python_exe} -m pip install -r requirements-xpu.txt", "Installing XPU requirements")
                break
    except:
        # Try alternative GPU detection
        result = subprocess.run("wmic path win32_VideoController get name", shell=True, capture_output=True, text=True, check=False)
    ...
    if (PROJECT_ROOT / "requirements-test.txt").exists():
        run_command(f"{python_exe} -m pip install -r requirements-test.txt", "Installing test requirements")
...
def main():
    ...
    print("2. Download data: python download_historical_data.py")
    print("3. Run experiments: python run_deep_learning.py")
    print("4. Start benchmark: python run_benchmark.py")
