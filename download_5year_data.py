...
def download_symbol_data(symbol, start_date, end_date, checkpoint):
    ...
    since = checkpoint.get(checkpoint_key, {}).get("last_timestamp", int(start_date.timestamp() * 1000))
    ...
    while since < end_timestamp:
        try:
            ...
            if len(all_data) % 10000 == 0:
                checkpoint[checkpoint_key] = {"last_timestamp": since, "records": len(all_data), "completed": False}
    ...
    # Mark as completed
    checkpoint[checkpoint_key] = {"last_timestamp": since, "records": len(df), "completed": True, "file": str(output_file)}
    save_checkpoint(checkpoint)
