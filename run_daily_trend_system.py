    ...
    def backtest(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        for i in range(200, len(df)):  # Start after we have 200-day SMA
            ...
            if position == 0:
                ...
                if (df.iloc[i]['sma_20'] > df.iloc[i]['sma_50'] and  # Short-term uptrend
                    df.iloc[i-1]['sma_20'] <= df.iloc[i-1]['sma_50'] and  # Just crossed
                    df.iloc[i]['close'] > df.iloc[i]['sma_200'] and  # Long-term uptrend
                    df.iloc[i]['rsi'] < 70 and  # Not overbought
                    df.iloc[i]['volume'] > df.iloc[i]['volume_sma'] * 1.2):  # Volume confirmation
...
def run_daily_trend_backtest():
    ...
    if results['trades']:
        ...
        for i, trade in enumerate(results['trades'][-5:], 1):
            print(f"Trade {i}: {trade['entry_time'].strftime('%Y-%m-%d')} -> "
