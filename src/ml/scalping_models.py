    ...
    def __init__(self, input_size: int, output_size: int, num_channels: List[int],
        ...
        for i in range(self.num_levels):
            ...
            self.tcn_blocks.append(
                self._make_tcn_block(in_channels, out_channels, kernel_size, 
...
class TransformerScalper(nn.Module):
    ...
    def __init__(self, input_size: int, d_model: int = 512, nhead: int = 8,
                 num_layers: int = 6, window_size: int = 100, dropout: float = 0.1):
        super().__init__()
...
class EnsembleMetaLearner(nn.Module):
    ...
    def __init__(self, models: Dict[str, nn.Module], meta_features: int = 10):
        ...
        self.meta_learner = nn.Sequential(
            nn.Linear(self.num_models + meta_features, 32),
            nn.ReLU(),
            nn.Dropout(0.2),
...
class ScalpingModelFactory:
    ...
    def create_ensemble(input_size: int, **kwargs) -> EnsembleMetaLearner:
        ...
        return EnsembleMetaLearner(
            models=models,
            meta_features=kwargs.get('meta_features', 10)
        ).to(DEVICE)
