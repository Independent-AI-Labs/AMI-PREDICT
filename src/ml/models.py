...
class LightGBMModel(BaseModel):
    ...
    def train(self, X: pd.DataFrame, y: pd.Series) -> dict[str, float]:
        ...
        # Train model
        self.model = lgb.train(self.params, train_data, valid_sets=[val_data], num_boost_round=100, callbacks=[lgb.early_stopping(10), lgb.log_evaluation(0)])
...
class CatBoostModel(BaseModel):
    ...
    def __init__(self, config: dict[str, Any]):
        super().__init__(config)
        self.params = {"iterations": 100, "learning_rate": 0.05, "depth": 6, "loss_function": "MAE", "random_seed": 42, "verbose": False}
...
class RandomForestModel(BaseModel):
    ...
    def train(self, X: pd.DataFrame, y: pd.Series) -> dict[str, float]:
        ...
        # Train model
        self.model = RandomForestRegressor(n_estimators=100, max_depth=10, min_samples_split=5, min_samples_leaf=2, random_state=42, n_jobs=-1)
...
class EnsembleModel:
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        self.weights = {"lightgbm": 0.3, "catboost": 0.3, "lstm": 0.2, "random_forest": 0.2}
        self.is_trained = False
        self.training_metrics = {}
    ...
    def save(self, path: str):
        ...
        metadata = {"weights": self.weights, "is_trained": self.is_trained, "training_metrics": self.training_metrics, "config": self.config}
