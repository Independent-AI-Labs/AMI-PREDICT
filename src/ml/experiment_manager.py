...
class DataLoader:
    ...
    def split_data(self, df: pd.DataFrame, train_end: str, val_end: str) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
...
class ExperimentRunner:
    ...
    def create_experiment(self, name: str, model_type: str, symbol: str, date_config: dict[str, str], hyperparameters: Optional[dict] = None) -> int:
    ...
    def run_experiment(self, experiment_id: int) -> dict[str, Any]:
        ...
        try:
            ...
            df = self.data_loader.load_price_data(exp["symbol"], exp["training_start"], exp["test_end"])

            # Engineer features
            df = self.data_loader.prepare_features(df)

            # Split data
            train_df, val_df, test_df = self.data_loader.split_data(df, exp["training_end"], exp["validation_end"])
            ...
            results = self._train_model(exp["model_type"], X_train, y_train, X_val, y_val, exp.get("hyperparameters"))
            ...
            # Save predictions
            predictions_df = pd.DataFrame({"timestamp": test_df["timestamp"].values, "symbol": exp["symbol"], "prediction": test_pred, "actual": y_test})
    ...
    def _train_model(
        self, model_type: str, X_train: np.ndarray, y_train: np.ndarray, X_val: np.ndarray, y_val: np.ndarray, hyperparameters: Optional[dict] = None
    ) -> dict[str, Any]:
    ...
    def compare_experiments(self, experiment_ids: list[int]) -> pd.DataFrame:
        ...
        pivot = comparison.pivot_table(index=["id", "name", "model_type"], columns=["dataset", "metric_name"], values="metric_value")

        return pivot

    def run_time_range_experiment(self, model_type: str, symbol: str, time_ranges: list[int]) -> list[int]:
        ...
        for months in time_ranges:
            ...
            date_config = {"train_start": start_date, "train_end": val_date, "val_end": end_date, "test_end": end_date}
