...
class RegimeDetector:
    ...
    def detect_regime(self, market_data: pd.DataFrame) -> tuple[MarketRegime, float]:
        ...
        # Update history
        self.regime_history.append({"timestamp": datetime.now(), "regime": regime, "confidence": confidence, "features": features})
    ...
    def _calculate_regime_features(self, market_data: pd.DataFrame) -> dict[str, float]:
        ...
        features["volatility_ratio"] = (
            float(returns.rolling(10).std().iloc[-1] / returns.rolling(50).std().iloc[-1]) if returns.rolling(50).std().iloc[-1] > 0 else 1.0
        )
        ...
        features["volume_trend"] = float((market_data["volume"].iloc[-1] - volume_sma.iloc[-1]) / volume_sma.iloc[-1]) if volume_sma.iloc[-1] > 0 else 0
        ...
        # Mean reversion
        features["mean_reversion"] = float((market_data["close"].iloc[-1] - sma_20.iloc[-1]) / market_data["close"].iloc[-1])
    ...
    def _classify_regime(self, features: dict[str, float]) -> MarketRegime:
        ...
        if (
            ...
            and features["trend_consistency"] > 0.01
            and features["higher_highs"] > features["lower_lows"]
            and features["momentum"] > 0.02
        ):
        ...
        if (
            features["trend_strength"] < -0.02
            and features["trend_consistency"] < -0.01
            and features["lower_lows"] > features["higher_highs"]
            and features["momentum"] < -0.02
        ):
