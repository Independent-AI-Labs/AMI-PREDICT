...
class CatBoostTrainer:
    ...
    def get_default_params(self) -> dict[str, Any]:
        ...
        if self.task_type == "classification":
            base_params.update({"loss_function": "Logloss", "eval_metric": "AUC", "auto_class_weights": "Balanced"})
        else:
            base_params.update({"loss_function": "RMSE", "eval_metric": "RMSE"})
    ...
    def train(
        ...
        eval_pool = None
        if X_val is not None and y_val is not None:
            eval_pool = cb.Pool(data=X_val, label=y_val, cat_features=cat_features)
        ...
        # Train model
        self.model.fit(train_pool, eval_set=eval_pool, plot=False, use_best_model=True if eval_pool else False)
