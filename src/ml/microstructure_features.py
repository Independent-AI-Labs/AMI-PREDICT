...
class MicrostructureFeatureEngine:
    ...
    def __init__(self, lookback_periods: dict[str, int] = None):
        ...
        """
        self.lookback_periods = lookback_periods or {"tick_momentum": 10, "order_flow": 20, "vwap": 100, "volume_profile": 500, "microstructure_noise": 50}
    ...
    @staticmethod
    @jit(nopython=True)
    def _calculate_order_flow_imbalance(bid_volumes: np.ndarray, ask_volumes: np.ndarray) -> float:
        """
    ...
    @staticmethod
    @jit(nopython=True)
    def _calculate_trade_size_clustering(volumes: np.ndarray, n_clusters: int = 3) -> np.ndarray:
        """
    ...
    def calculate_microstructure_features(self, ohlcv_data: pd.DataFrame, order_book_data: Optional[pd.DataFrame] = None) -> pd.DataFrame:
        """
        ...
        features["vwap"] = pd.Series(prices).rolling(self.lookback_periods["vwap"]).apply(lambda x: self._calculate_vwap(x.values, volumes[-len(x) :]))
        features["vwap_deviation"] = (prices - features["vwap"]) / features["vwap"]

        # 2. Tick momentum
        features["tick_momentum"] = pd.Series(prices).rolling(self.lookback_periods["tick_momentum"]).apply(lambda x: self._calculate_tick_momentum(x.values))

        # 3. Volume profile features
        features["volume_mean"] = pd.Series(volumes).rolling(self.lookback_periods["volume_profile"]).mean()
        features["volume_std"] = pd.Series(volumes).rolling(self.lookback_periods["volume_profile"]).std()
        ...
        if order_book_data is not None and len(order_book_data) > 0 and "bid" in order_book_data.columns:
            ...
            features["order_flow_imbalance"] = (
                pd.Series(bid_vols)
                .rolling(self.lookback_periods["order_flow"])
                .apply(lambda x: self._calculate_order_flow_imbalance(x.values, ask_vols[-len(x) :]))
            )
        ...
        # ATR for volatility
        features["atr_14"] = talib.ATR(ohlcv_data["high"].values, ohlcv_data["low"].values, prices, timeperiod=14)
        ...
        features["ad"] = talib.AD(ohlcv_data["high"].values, ohlcv_data["low"].values, prices, volumes)
        ...
        features["efficiency_ratio"] = pd.Series(prices).rolling(10).apply(efficiency_ratio, raw=False)
    ...
    def calculate_regime_features(self, ohlcv_data: pd.DataFrame) -> pd.DataFrame:
        ...
        features["adx"] = talib.ADX(ohlcv_data["high"].values, ohlcv_data["low"].values, prices, timeperiod=14)
        ...
        features["trend_strength"] = pd.Series(prices).rolling(20).apply(lambda x: trend_strength(x.values), raw=False)
        ...
        features["regime_trend"] = np.where(features["trend_strength"] > 0.001, 1, np.where(features["trend_strength"] < -0.001, -1, 0))
        features["regime_volatility"] = np.where(features["realized_vol"] > features["realized_vol"].rolling(100).mean(), 1, 0)

        return features

    def _calculate_hurst_exponent(self, returns: pd.Series, lags: list[int] = None) -> pd.Series:
        """
    ...
    def update_buffers(self, price: float, volume: float, bid: Optional[float] = None, ask: Optional[float] = None):
        """
...
def test_feature_engine():
    ...
    for i in range(100):
        engine.update_buffers(price=prices[i], volume=ohlcv_data["volume"].iloc[i], bid=order_book_data["bid"].iloc[i], ask=order_book_data["ask"].iloc[i])
