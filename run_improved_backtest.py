...
from src.ml.backtester import Backtester, Portfolio
...
def generate_smart_signals(model: torch.nn.Module, df: pd.DataFrame, 
    ...
    for i, (idx, pred) in enumerate(zip(indices, predictions)):
        ...
        if current_position is not None:
            ...
            if (pnl_pct >= trading_system.take_profit or  # Take profit
                pnl_pct <= -trading_system.stop_loss or  # Stop loss
                bars_held >= trading_system.min_holding_period * 2):  # Max holding
        ...
        # Check if we should open position
            ...
            # Additional filters
            if (regime in ['trending_up', 'ranging'] and  # Favorable regime
                volatility < 0.03 and  # Not too volatile
                trading_system.check_risk_limits(idx)):  # Within risk limits
...
def run_improved_backtest(
    ...
    results = backtester.run(
        data=aligned_df,
        signals=signals,
        position_size=trading_system.max_position_size
    )
